                ...
                <form action="{{ route('auth.register.post') }}" method="POST" id="formRegister" class="my-6">
                    ...
                    <div class="flex">
                        <input type="text" required name="name"
                            class="w-full mr-1 backdrop-blur-xs placeholder-white font-normal bg-white bg-opacity-50 text-white my-4 p-4 px-6 rounded-full"
                            value="{{old("name")}}"
                            placeholder="Nama Lengkap">
                        <input type="text" required name="email"
                            class="w-full ml-1 backdrop-blur-xs placeholder-white font-normal bg-white bg-opacity-50 text-white my-4 p-4 px-6 rounded-full"
                            value="{{old("email")}}"
                            placeholder="Username/Email">
                    </div>
                    <input type="number" required name="phonenumber"
                        class="w-full backdrop-blur-xs placeholder-white font-normal bg-white bg-opacity-50 text-white my-4 p-4 px-6 rounded-full"
                    ...
                    <button onclick="loginForm(this, event)"
                        class="mt-6 text-xl bg-primary w-full p-3 py-4 text-center rounded-full text-white">
                        Daftar
                    </button>
                    <p class="text-md mt-4 text-center w-full text-white"> Sudah punya akun ? <a
                            href="{{ route('auth.login') }}" class="font-bold text-primary">&nbsp;Masuk</a> </p>
                </form>
            </div>
        </div>
    </div>
